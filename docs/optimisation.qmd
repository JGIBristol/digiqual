...
# A simple physics model: Signal = Length * 2 + Noise
# We use python -c to run this as if it were an external CLI tool.
SOLVER_CMD = (
    f"{sys.executable} -c "
    "'import pandas as pd, numpy as np; "
    'df=pd.read_csv("{input}"); '
    'df["Signal"] = df["Length"] * 2.0 + np.random.normal(0, 1, len(df)); '
...
1. Generates 10 initial LHS points (because study.data is empty).

2. Runs the solver.

3. Checks diagnostics.

4. If needed, generates 5 new points and repeats (up to 3 times).

```{python}
study.optimise(
    command=SOLVER_CMD,
    ranges=ranges,
    n_start=15,  # Initial batch size
    n_step=5,  # Refinement batch size
...
_ = study.pod(poi_col="Length", threshold=4)
study.visualise()
```
