...
*Note: Instead of using the complex analytical formulas from MIL-HDBK-1823A, we are using modern bootstrapping to evaluate the confidence bounds. This strictly maintains the classical assumptions (constant variance and normal errors) while easily calculating the worst-case curve. Under the hood, `linear_pod` automatically computes bounds for multiple confidence levels (50%, 90%, 95%, 99%) and saves them to the `reliability_table` and `ci_bounds` dictionaries in the results.*
...
Now, we will run the generalised method. This approach uses cross-validation to select the best non-linear model (Polynomial or Kriging) and automatically models the changing variance using kernel smoothing. Like the classical path, the bootstrap process computes bounds across all standard confidence levels (50%, 90%, 95%, 99%) simultaneously.
