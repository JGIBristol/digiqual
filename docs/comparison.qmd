...
def apply_physics(df):
    ...
    signal = 2.0 + (1.5 * df["Length"]) + (0.3 * df["Length"]**2)

    # 2. Heteroscedastic Noise: Larger flaws = More scatter
    noise_scale = 0.2 + (0.5 * df["Length"])
...
df = pd.DataFrame({
    "Length": np.random.uniform(0.5, 10.0, 80)
})
...
relaxed_results = study.pod(poi_col="Length", threshold=15.0, n_jobs=-1)
