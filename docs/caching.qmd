...
**What it does:** Caches the heteroscedastic noise profile and parameter sensitivity results.
**Trigger:** Resolving a specific mean model.

Once a mean model is selected, the engine must estimate how the noise (variance) changes across the surface. It uses Leave-One-Out Cross-Validation (LOO-CV) to find the optimal smoothing bandwidth for the residuals, and tests candidate statistical distributions to find the best fit. In addition, the engine computes the Total-Order Sobol Sensitivity Indices to quantify parameter importance.

* **Cache Key:** Inherits the Mean Model key (e.g., `('Polynomial', 3)`).
* **Stored Data:** `residuals`, `bandwidth`, `dist_info` (name and parameters), and `sobol_indices`.
* **Benefit:** Bandwidth optimization and Sobol sensitivity computations are slow. By caching them, the system never has to recalculate the noise profile or parameter sensitivities for a model it has already analyzed.
...
4. It calls `bootstrap_pod_ci()`, firing up parallel CPU cores to resample the data 1,000 times, refitting the models and running strict Layer 3 math on every iteration to build robust Confidence Bounds for a range of levels (50%, 90%, 95%, 99%) simultaneously. These bounds are stored in a `ci_bounds` dictionary and a full `reliability_table` matrix.
